...
local function check_plugin_updates()
	...
	if ya and ya.notify then
		ya.notify({
			title = "Lis",
			content = "Halo, slemat datang di Lisa Manager!",
			timeout = 3,
		})
	end
end

-- Override status bar to hide progress bar
if Status then
	function Status:render()
		local left = ui.Line {}
		local right = ui.Line {}
		
		-- Add mode indicator to left
		left = left:append(self:mode())
		left = left:append(self:tabs())
		
		-- Add position to right (no progress)
		right = right:append(self:position())
		
		return {
			ui.Paragraph(left, ui.Constraint.Length(50)),
			ui.Paragraph(right, ui.Constraint.Length(20)),
		}
	end
end
